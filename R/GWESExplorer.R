...
#' @param max_request_size Maximum web request size in bytes, which limits file uploads. Defaults to 32 MiB.
#'                         This changes the global shiny.maxRequestSize option, which resets when the R session ends.
#'
...
launch_GWESExplorer <- function(launch_in_browser = TRUE, max_request_size = 32 * 1024 * 1024) {
    ...
    shiny::shinyApp(.create_ui_layout(), .server, options = list(launch.browser = launch_in_browser))
}
